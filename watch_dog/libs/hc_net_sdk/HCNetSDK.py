...
# NET_DVR_COMPRESSION_AUDIO 参数
class NET_DVR_COMPRESSION_AUDIO(ctypes.Structure):
    _fields_ = [
        ("byAudioEncType", c_byte),
        # 音频编码类型：0- G722，1- G711_U，2- G711_A，5- MP2L2，6- G726，7- AAC，
        # 8- PCM，9-G722，10-G723，11-G729，12-AAC_LC，13-AAC_LD，
        # 14-Opus，15-MP3，16-ADPCM
        ("byAudioSamplingRate", c_byte),
        # 音频采样率：0- 默认，1- 16kHZ，2- 32kHZ，3- 48kHZ，4- 44.1kHZ，5- 8kHZ
        ("byAudioBitRate", c_byte),
        # 音频码率，枚举类型
        # BITRATE_ENCODE_def = 0, // 默认
        # BITRATE_ENCODE_8kps = 1, // 8  kbs
        # BITRATE_ENCODE_16kps = 2, // 16  kbs
        # BITRATE_ENCODE_32kps = 3, // 32  kbs
        # BITRATE_ENCODE_64kps = 4, // 64  kbs
        # BITRATE_ENCODE_128kps = 5, // 128  kbs
        # BITRATE_ENCODE_192kps = 6, // 192  kbs
        # BITRATE_ENCODE_40kps = 7, // 40  kbs
        # BITRATE_ENCODE_48kps = 8, // 48  kbs
        # BITRATE_ENCODE_56kps = 9, // 56  kbs
        # BITRATE_ENCODE_80kps = 10, // 80  kbs
        # BITRATE_ENCODE_96kps = 11, // 96  kbs
        # BITRATE_ENCODE_112kps = 12, // 112  kbs
        # BITRATE_ENCODE_144kps = 13, // 144  kbs
        # BITRATE_ENCODE_160kps = 14, // 160  kbs
        # BITRATE_ENCODE_224kps = 15, // 224  kbs
        # BITRATE_ENCODE_256kps = 16, // 256  kbs
        # BITRATE_ENCODE_320kps = 17 // 320  kbs
        ("byres", c_byte * 4),  # 保留，置为0
        ("bySupport", c_byte)  # Mp2l2前4个字节的含义表示后面内容音频数据长度
    ]
...
class NET_DVR_AUDIOENC_INFO(Structure):
    _fields_ = [
        ("in_frame_size", c_int),
        ("reserved", c_byte * 16),
    ]
...
LPNET_DVR_AUDIOENC_INFO = POINTER(NET_DVR_AUDIOENC_INFO)


class NET_DVR_AUDIOENC_PROCESS_PARAM(Structure):
    _fields_ = [
        ("in_buf", POINTER(c_byte * 640)),  # 输入数据缓冲区，存放编码前PCM原始音频数据
        ("out_buf", POINTER(c_byte * 320)),  # 输出数据缓冲区，存放编码后音频数据
        ("out_frame_size", c_int),
        # 编码后一帧音频数据的大小(单位为字节)：如果是G711编码，输入数据的大小为160字节；
        # 如果是G722编码，输入数据的大小为80字节
        ("g726enc_reset", c_int),  # G726重置开关(保留)
        ("g711_type", c_int),  # G711编码类型：0- U law，1- A law
        ("enc_mode", c_int),  # AMR音频编码模式配置(保留)
        ("reserved", c_byte * 16),
    ]


LPNET_DVR_AUDIOENC_PROCESS_PARAM = POINTER(NET_DVR_AUDIOENC_PROCESS_PARAM)


class NET_DVR_AUDIODEC_INFO(Structure):
    _fields_ = [
        ("nchans", c_uint32),  # 声道数
        ("sample_rate", c_uint32),  # 采样率
        ("aacdec_profile", c_uint32),  # 编码用的框架
        ("reserved", c_byte * 16),
    ]
...
class NET_DVR_AUDIODEC_PROCESS_PARAM(Structure):
    _fields_ = [
        ("in_buf", POINTER(c_char * 160)),  # 输入数据缓冲区，存放编码前PCM原始音频数据
        ("out_buf", POINTER(c_char * 320)),  # 输出数据缓冲区，存放编码后音频数据
        ("in_data_size", c_uint32),
        ("proc_data_size", c_uint32),
        ("out_frame_size", c_uint32),
        ("dec_info", POINTER(NET_DVR_AUDIODEC_INFO)),
        ("g726dec_reset", c_uint32),  # G726重置开关(保留)
        ("g711_type", c_uint32),  # G711编码类型：0- U law，1- A law
        ("reserved", c_byte * 16),
    ]


LPNET_DVR_AUDIODEC_PROCESS_PARAM = POINTER(NET_DVR_AUDIODEC_PROCESS_PARAM)
...
# 异步登录回调函数
fLoginResultCallBack = CFUNCTYPE(None, c_uint32, c_uint32,
                                 LPNET_DVR_DEVICEINFO_V30, c_void_p)
...
class NET_DVR_ALARMINFO_V30(Structure):
    _fields_ = [
        ...
        ("byDiskNumber",
         c_byte * 33)]  # 发生报警的硬盘。当报警类型为1，4，5时有效，byDiskNumber[0]=1表示1号硬盘异常
...
class NET_DVR_SETUPALARM_PARAM(Structure):
    _fields_ = [
        ...
        # 0- 表示二进制传输，1- 表示URL传输（设备支持的情况下，设备支持能力根据具体报警能力集判断,同时设备需要支持URL的相关服务，当前是”云存储“）
        ("byCustomCtrl",
         c_byte)]  # Bit0- 表示支持副驾驶人脸子图上传: 0-不上传,1-上传,(注：只在公司内部8600/8200等平台开放)
...
class NET_DVR_ACS_EVENT_INFO(Structure):
    _fields_ = [
        ...
        ("byCardType", c_byte),
        # 卡类型：1- 普通卡，2- 残障人士卡，3- 非非授权名单卡，4- 巡更卡，5- 胁迫卡，6- 超级卡，7- 来宾卡，8- 解除卡，为0表示无效
...
class NET_DVR_ACS_ALARM_INFO(Structure):
    _fields_ = [
        ...
        ("byRes2", c_byte),  # 保留，置为0
        ("byAcsEventInfoExtendV20", c_byte),
...
class NET_DVR_ID_CARD_INFO_ALARM(Structure):
    _fields_ = [
        ...
        ("byCardType", c_ubyte),
        # 卡类型，1-普通卡，2-残疾人卡，3-非非授权名单卡，4-巡更卡，5-胁迫卡，6-超级卡，7-来宾卡，8-解除卡，为0无效
        ...
        ("pIDCardInfoExtend", POINTER(NET_DVR_ID_CARD_INFO_EXTEND)),
        # 身份证刷卡扩展事件信息
...
# 报警信息回调函数
MSGCallBack_V31 = fun_ctype(c_bool, c_uint32, LPNET_DVR_ALARMER, c_void_p,
                            c_ulong, c_void_p)
MSGCallBack = fun_ctype(None, c_uint32, LPNET_DVR_ALARMER, c_void_p, c_ulong,
                        c_void_p)
# 码流回调函数
REALDATACALLBACK = fun_ctype(None, c_long, c_ulong, POINTER(c_ubyte), c_ulong,
                             c_void_p)
